...
export default function SingleBlog({ post }) {
    const { title, content, featuredImg, created_at, updated_at, readingTime, author } = post || {};
    return (
        <>
                ...
                <div className="text-gray-500 flex justify-center items-center space-x-2">
                    <span className="flex space-x-2 items-center overflow-hidden">
                        <img
                            ...
                            src={
                                author.data.attributes.image.data.attributes.formats.thumbnail.url
                                    ? `http://localhost:1337${author.data.attributes.image.data.attributes.formats.medium.url}?resize=32x32`
            ...
            <div className="rounded-md h-56 w-full overflow-hidden">
                <img
                    ...
                    src={
                        featuredImg ? `http://localhost:1337${featuredImg.data.attributes.formats.large.url}` : 'https://via.placeholder.com/1080'
                    }
                />
            </div>
